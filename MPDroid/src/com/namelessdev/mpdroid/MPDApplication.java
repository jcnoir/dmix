...
public class MPDApplication extends Application implements ConnectionListener {
    ...
    private AlertDialog settingsConnectionFailureAlertDialog;
    private AlertDialog connectionFailureAlertDialog;
    ...
    private void connectMPD() {
        ...
        if (currentActivity != null) {
            connectingAlertDialog = new ProgressDialog(currentActivity);
            connectingAlertDialog.setTitle(getResources().getString(R.string.connecting));
            connectingAlertDialog.setMessage(getResources().getString(R.string.connectingToServer));
            connectingAlertDialog.setCancelable(false);
            connectingAlertDialog.setOnKeyListener(new OnKeyListener() {
    ...
    public void connectionFailed(String message) {
        ...
        if (currentActivity != null && connectionLocks.size() > 0) {
            ...
            if (currentActivity.getClass() == SettingsActivity.class) {
                ...
                settingsConnectionFailureAlertDialog = builder.show();
            } else {
                AlertDialog.Builder builder = new AlertDialog.Builder(currentActivity);
                ...
                try {
                    connectionFailureAlertDialog = builder.show();
                } catch (BadTokenException e) {
                    // Can't display it. Don't care.
                }
            }
    ...
    private void dismissAlertDialog(AlertDialog ad) {
        if (ad != null) {
            if (ad.isShowing()) {
                try {
                    ad.dismiss();
    ...
    private void dismissAlertDialogs() {
        dismissAlertDialog(settingsConnectionFailureAlertDialog);
        dismissAlertDialog(connectingAlertDialog);
        dismissAlertDialog(connectingAlertDialog);
    }
